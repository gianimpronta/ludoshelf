...
const usadoPorCompartimento = (arranjo: Arranjo, ctx: ReturnType<typeof montarContexto>) => {
...
describe('melhorar', () => {
  ...
  it('nunca estoura a largura de um compartimento', () => {
    ...
    for (const total of usadoPorCompartimento(resultado, ctx).values()) {
  ...
  it('aloca um jogo que ficou de fora quando ha lugar para ele', () => {
    ...
    const resultado = melhorar(comPendente, ctx, PESOS_PADRAO, geradorMulberry32(5), 500)
  ...
  it('nao desaloca ninguem que ja estava posicionado', () => {
    ...
    const ctx = montarContexto(jogos, estante)
    const inicial = montarArranjoInicial(jogos, ctx)
    const resultado = melhorar(inicial, ctx, PESOS_PADRAO, geradorMulberry32(13), 2000)
